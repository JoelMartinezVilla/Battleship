...
ssh -i $RSA_PATH -t -p $SERVER_PORT "$USER@ieticloudpro.ieti.cat" $sshCommand
...
ssh -i $RSA_PATH -t -p $SERVER_PORT "$USER@ieticloudpro.ieti.cat" "sudo iptables -t nat -L -n -v"
